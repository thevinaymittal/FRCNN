...
print(f'Num classesabra= {len(classes_count)}')
print(f'Num classesabra = {len(class_mapping)}')
...
data_gen_train = data_generators.get_anchor_gt(train_imgs, classes_count, C, nn.get_img_output_length, keras.backend.image_data_format(), mode='train')
data_gen_val = data_generators.get_anchor_gt(val_imgs, classes_count, C, nn.get_img_output_length,keras.backend.image_data_format(), mode='val')
...
for epoch_num in range(num_epochs):
	...
	while True:
		try:
			...
			R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, keras.backend.image_data_format(), use_regr=True, overlap_thresh=0.7, max_boxes=300)
